...
public record CountedLoopStmt(LocalAllocator.LocalBinding index, LongExpr endExclusive, Stmt body) implements Stmt {
    /*
     * Emits:
     *
    ...
    public void emit(CodeEmitter out) {
        ...
        new IfStmt(
                ...
                compareLong(NumericExpr.asLong(new LocalExpr(index)), endExclusive),
                null,
                loopDone
        ).emit(out);
    ...
    private static Expr compareLong(LongExpr left, LongExpr right) {
        return out -> {
            left.emit(out);
            right.emit(out);
            out.lcmp();
