...
public record MemLayout(MemoryLayout layout, Optional<List<MemoryLayout>> groupLayouts) implements LayoutRules{
    ...
    /// @throws IllegalArgumentException if the record declares an invalid
    ///         {@link align} value
    ...
    private static MemLayout of(RecordField field, Optional<List<MemoryLayout>> groupLayoutLists){        
        ...
        for (RecordComponent component : components) {
            switch (FieldType.of(component)) {
                ...
                case RecordField rec -> {
                    ...
                    // Update the offset
                    offset = alignedOffset + rSize.size();
                }
        ...
        MemoryLayout layout = MemoryLayout.structLayout(layArray);
        long alignment = field.alignByteSize();
        if (alignment > 0) {
            layout = layout.withByteAlignment(alignment);
        }
        layout = layout.withName(field.type().getSimpleName());
        return new MemLayout(layout, groupLayoutLists);
    }
